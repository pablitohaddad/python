...
class music_cog(commands.Cog):
    def __init__(self, bot):
        ...
        self.FFMPEG_OPTIONS = {"before_options": "=reconnect 1 -reconnected_streamed 1 -reconnected_delay_max 5", "options": "-vn"}
    ...
    def search_yt(self, item):
        with YoutubeDL(self.YDL_OPTIONS) as ydl:
            try:
                ...
                return False
        return {'source': info['formats'[0]['url']], 'tittle': info['tittle']}
    ...
    async def play_music(self, ctx):
        if len(self.music_queue) > 0:
            ...
            if self.vc == None or not self.vc.is_connectec():
    ...
    @commands.command(name="queue", aliases=["q"], help="Displays all the song currently in the queue")
    async def queue(self, ctx):
        retval = ""

        for i in range(0, len(self.music_queue)):
            if i > 4: break
            retval += self.music_queue[i][0]["tittle"] + "\n"
